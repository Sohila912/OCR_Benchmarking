...
Python can only execute a program if the syntax is correct; otherwise, the interpreter displays an error message. Syntax refers to the structure of a program and the rules about that structure. For example, parentheses have to come in matching pairs, so ✭✶ ✰ ✷✮ is legal, but ✽✮ is a syntax error .
...
The parentheses indicate that ♣/a114✐♥/a116 is a function. We'll get to functions in Chapter 3.
