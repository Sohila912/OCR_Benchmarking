2101.03961v3 [cs.LG] 16 Jun 2022
...
2.1 Simplifying Sparse Routing ................. 2.000000 8 | 5
2.2 Efficient Sparse Routing ..... 2... 2.20.02... 002 ee ee 6
...
3.1 Scaling Results on a Step-Basis ........0.0.0.0.0.0 2.000002 ee eee 12
3.2 Scaling Results on a Time-Basis.................2..-002000. 13
3.3 Scaling Versus a Larger Dense Model... ................04. 13
4 Downstream Results 14
4.1 Fine-Tuning.... 2... 2.0... 2 ee ee 14
4.2 Distillation ©... 2... 2. ee 16
4.3 Multilingual Learning ...........0..0... 0.020020. 200 20000. 17
5 Designing Models with Data, Model, and Expert-Parallelism 18
5.1 Data Parallelism .. 2... 0... 20
5.2 Model Parallelism .... 2... 0. 20
5.3 Model and Data Parallelism... 2... ...0.0.0.20.0..02.. 2020000. 21
5.4 Expert and Data Parallelism ..........2....2........2004. 22
5.5 Expert, Model and Data Parallelism ..................20004 22
5.6 Towards Trillion Parameter Models... ..............-..-000. 22
6 Related Work 24
7 Discussion 25
8 Future Work 26
9 Conclusion 27
...
te -1.2
6.04 @ — Switch-Base: 128e
\ 2e 1.34 —— Switch-Base: 64e
x — Switch-Base: 32¢
5.84 + —— Switch-Base: 16e
\4e -1.47 —— T5-Base
a
\ >
5.64 * 454
...
4.8 * 20
, 169 ide “0 1 2 3 4
Sparse Model Parameters Training Step ted
...
e The Switch Transformer architecture, which simplifies and improves over Mixture of
Experts.

e Scaling properties and a benchmark against the strongly tuned T5 model (Raffel et al.,
...
e Successful distillation of sparse pre-trained and specialized fine-tuned models into
...
e Improved pre-training and fine-tuning techniques: (1) selective precision training that
...
e A measurement of the pre-training benefits on multilingual data where we find a
...
e An increase in the scale of neural language models achieved by efficiently combining
data, model, and expert-parallelism to create models with up to a trillion parameters.
These models improve the pre-training speed of a strongly tuned T5-X XL baseline by
Ax.
...
sequence. We diagram two tokens (x; = “More” and x2 = “Parameters” below)
...
this to the best determined top-k experts, selected from a set {E;(a)}®, of N experts.
The router variable W,. produces logits h(a) = W,- a which are normalized via a softmax
distribution over the available N experts at that layer. The gate-value for expert i is given
by,
e(2)i

pi(x) = SN ches (1)
The top-k gate values are selected for routing the token x. If 7 is the set of selected top-k
...
Routing, the gate value p;(x) in Equation 2 permits differentiability of the router.
...
each expert can be at least halved since each token is only being routed to a single expert.®
...
each having their own unique : er
...
(total_tokens / num_experts) x capacity_factor. If the tokens are unevenly dis-
...
expert capacity -( ) x capacity factor. (3)
...
where f; is the fraction of tokens dispatched to expert 7,

f= 7D Margmax p(x) =i} (5)

xeEB

and P; is the fraction of the router probability allocated for expert i, ?
...
2. A potential source of confusion: p;(x) is the probability of routing token x to expert i. P; is the
probability fraction to expert 7 across all tokens in the batch B.

FEDuS, ZOPH AND SHAZEER
...
routing hi -P) = yeas . x) = x: Finally, a hyper-parameter a is a multiplicative
coefficient for these auxiliary losses; throughout this work we use an a = 10~? which was
sufficiently large to ensure load balancing while small enough to not to overwhelm the
primary cross-entropy objective. We swept hyper-parameter ranges of a from 107! to 10~°
in powers of 10 and found 107? balanced load quickly without interfering with training loss.
...
negative log perplexity. Throughout all tables in the paper, + indicates that a higher value
...
Model Capacity Quality after Time to Quality Speed (tT)
Factor 100k steps (t) Threshold (|) | (examples/sec)
(Neg. Log Perp.) (hours)
...
are recast to bfloatl6 precision at the end of the function, no expensive float32 tensors

