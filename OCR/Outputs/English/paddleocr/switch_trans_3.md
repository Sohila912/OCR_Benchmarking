...
precision (bloat16) formats. We design models based off T5-Base and T5-Large (Raffel
...
at http://jm1r.org/papers/v23/21-0998.htm1.
...
this to the best determined top-k experts, selected from a set {E;(x)}1 of N experts.
The router variable W, produces logits h(x) = W, . x which are normalized via a softmax
...
Expert 2
Expert 3
Experts: Split across dev
each having their own uniqu.
parameters. Perform standard feed
.Expert Capacity: Batch size o
each expert. Calculated as
(tokens_per
Across Device
capacity_factor
Capacity Factor: Used wher.
...
(total_tokens / num_experts)  capacity_factor. If the tokens are unevenly dis-
...
(5)
xEB
and P, is the fraction of the router probability allocated for expert i, 2
(6)
xEB
...
coefficient for these auxiliary losses; throughout this work we use an a = 10-2 which was
sufficiently large to ensure load balancing while small enough to not to overwhelm the
primary cross-entropy objective. We swept hyper-parameter ranges of  from 10-1 to 10-5
...
level threshold of -1.50. t T5-Base did not achieve this negative log perplexity in
...
foat32 precision throughout their MoE Transformer. However, we show that by instead
selectively casting to float32 precision within a localized part of the model, stability may be
achieved, without incurring expensive communication cost of foat32 tensors. This technique
...
that our approach permits nearly equal speed to bfoat16 training while conferring the
